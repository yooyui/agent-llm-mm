...
use agent_llm_mm::support::packaging_preflight::{
    PackagingPreflightOptions, summarize_packaging_preflight,
};
...
fn print_usage() {
    eprintln!(
        "usage: packaging_preflight_check --release-candidate <name> [--evidence-root <path>] [--output-json <path>] [--output-md <path>]"
    );
}
