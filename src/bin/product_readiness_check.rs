...
use agent_llm_mm::support::product_readiness::{
    ProductReadinessOptions, summarize_product_readiness,
};
...
fn print_usage() {
    eprintln!(
        "usage: product_readiness_check --release-candidate <name> [--evidence-root <path>] [--output-json <path>] [--output-md <path>]"
    );
}
