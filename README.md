...
- Automatic identity revision replaced the old global-count heuristic with distinct episodes reached through selected active-claim IDs, persisted `evidence_links`, and `episode_events`. Unrelated global episodes no longer raise the support count. The bounded join does not yet scope-bind both Claim and Event endpoints; M1.0.1 tracks that data-integrity gap before further M1 feature expansion.
...
- Three scope/data-integrity gates remain open before later M1 feature expansion: the identity supporting-Episode query is not yet fully scope-bound, ordinary Claim provenance does not yet redact every mixed-scope revision edge in full, and legacy-valid `Owner::Unknown` world/project rows do not yet have a frozen scoped write-read reachability contract.
...
- Scope/data-integrity gates for identity evidence-to-Episode counting, mixed-scope Claim revision-edge redaction, and owner/namespace write-read reachability
