...
- The runtime read interface now covers complete event and claim search/lookup records plus a bounded Claim-linked reflection-history slice. The history path filters evidence references to the same scope and remains read-only/provider-free, but it does not cover identity/commitment history, record-only reflections, episode/reflection `get_memory`, or correction tools. It adds no schema migration or index and retains an MVP table-scan performance boundary.
...
- Complete episode/reflection read models, identity/commitment and record-only reflection history, and audited correction tools
