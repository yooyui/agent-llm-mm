...
| `M1` | M1.1.1 Scoped Event Recall Read Model | `implemented` | The additive `search_memory` path requires an explicit namespace and returns bounded recent-first complete event records with stable IDs, timestamps, scope, kind, summary, and persisted claim/episode links. It remains event-only and is not a complete cross-record read model, text search, history, or correction flow. | Preserve SQL-level owner+namespace filtering before limit, exact-ID no-widening, provider-independent reads, and semantic-table read-only behavior. Keep claim/episode/reflection/history/supersession work in later slices. | focused `sqlite_store` event-recall tests; focused `mcp_stdio` search tests including reconnect with an offline provider; `cargo test --test status_sync`; `./scripts/status-sync-check.sh`. |
