...
| `P2` | Structured decision protocol | `implemented` | `decide_with_snapshot` has a v2 response envelope with `decision_id`, requested/selected action, local confidence metadata, policy checks, and non-claims, but the protocol envelope does not make policy integrity complete: the caller supplies the snapshot, and the provider-selected action is not re-gated before return. The model decision remains a minimal action string. This is not a full decision engine. | Preserve legacy `blocked` / `decision` callers while adding any future schema fields. Before widening the protocol, load the authoritative scoped snapshot inside the use case and re-check the selected action against the same policy boundary. Do not add provider JSON decisions, calibrated confidence scoring, planning, or broader policy arbitration without separate tests and docs. | `docs/product/structured-decision-protocol.md`; `cargo test --test decision_flow -v`; adversarial tests for caller snapshot substitution and provider-selected action drift; future MCP schema compatibility checks when the wire schema changes. |
