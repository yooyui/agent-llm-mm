...
- `partial`: Provider matrix; `mock`, `openai-compatible`, and `openrouter`
  ...
  planned-only. OpenRouter is verified through local OpenAI-compatible
  `/chat/completions` stubs, not live-provider certification.
...
Current branch `cargo test` total declaration: 352 tests.
