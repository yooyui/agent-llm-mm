...
- `partial`: `decide_with_snapshot` now replaces caller commitments with the
  ...
  actions, but the remaining identity / claims / evidence / episodes are still
  caller-provided and there is no server-created snapshot handle or provenance
  binding.
- `partial`: the report returned by `doctor` is read-only, but entering the
  command currently uses runtime bootstrap and can create, migrate, or seed the
  configured database. The planned `doctor` / `init` / `migrate` command
  boundary is not implemented.
- `not-implemented`: migrations have no durable schema-version ledger or
  transactional recovery contract for legacy table rebuilds.
...
| `P2` | Structured decision protocol | `implemented` | `decide_with_snapshot` has a v2 response envelope with `decision_id`, requested/selected action, local confidence metadata, policy checks, and non-claims. The M0.3.1 slice binds commitments to the server store and re-gates the provider-selected action, while the remaining snapshot fields are still caller-provided and the model decision remains a minimal action string. This is not a full decision engine. | Preserve legacy `blocked` / `decision` callers and the provider action-string contract. Before widening the protocol, add a server-created trusted snapshot/provenance boundary; do not add provider JSON decisions, calibrated confidence scoring, planning, or broader policy arbitration without separate tests and docs. | `docs/product/structured-decision-protocol.md`; `cargo test --test decision_flow -v`; focused `mcp_stdio` tests for caller commitment removal and provider-selected action rejection; future MCP schema compatibility checks when the wire schema changes. |
...
| `P2` | Data lifecycle and migration | `partial` | Backup/restore local scripts are covered, but this is not remote backup, cloud sync, scheduled backup, or production disaster recovery. Legacy rebuilds also lack a durable schema-version ledger and an explicit transactional recovery contract. | Introduce versioned, restart-safe migrations and failure-injection tests before the next schema change; keep restore-to-new-path and manual database switch boundary. | `cargo test --test sqlite_backup_restore -v`; schema-version and interrupted-migration recovery tests for any future migration. |
