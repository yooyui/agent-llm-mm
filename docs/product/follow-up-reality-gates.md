...
- `partial`: Release engineering; local soak now writes machine-readable
  compatibility and release-boundary artifacts, but installer, binary package,
  service manager, auto-updater, Windows/fresh-machine evidence, Beta/GA
  evidence, and production-ready claims remain blocked.
...
| --- | --- | --- | --- | --- | --- |
| `P0` | Local Alpha evidence summary | `implemented` | Mainline previously had no single read-only rollup to distinguish open, not-verified, and satisfied gates. The summary now also exposes `external_blockers`, `human_blockers`, and `unimplemented_capability_blockers`. | Keep `scripts/local-alpha-evidence-summary.sh`, `src/bin/local_alpha_evidence_summary.rs`, `src/support/local_alpha_evidence.rs`, and `tests/local_alpha_release_evidence.rs` in the productization branch; run it after every release-gate refresh. Do not treat blocker rows as generated external evidence. | `cargo test --test local_alpha_release_evidence -v`; `cargo run --quiet --bin local_alpha_evidence_summary -- --evidence-root .` |
...
Current branch `cargo test` total declaration: 303 tests.
