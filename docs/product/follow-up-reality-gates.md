...
| `P0` | Local Alpha full release gate | `partial` | Product smoke evidence may exist locally, but support bundle, first-run, Windows parity, and release decision evidence can be missing or stale. | Re-run product smoke, support bundle generation, first-run simulation, evidence summary, and record dated gate results without claiming completion from old artifacts. | `./scripts/product-smoke-local.sh`; `./scripts/generate-support-bundle.sh target/support-bundles/local-alpha-gate`; `./scripts/first-run-bootstrap-smoke-local.sh target/first-run-bootstrap-smoke/local-alpha-gate`; evidence summary JSON showing every gate state. |
...
| `P1` | Observe-only daemon lifecycle | `partial` | `doctor.daemon_observe_only` is real, but `src/application/daemon.rs` still has a placeholder loop and no operational lifecycle proof. | Keep write capability blocked; add lifecycle design and observe-only start/stop checks before any daemon write path. | `cargo test --test daemon_config -v`; future lifecycle tests that prove no remote listener, no semantic writes, and clean shutdown. |
