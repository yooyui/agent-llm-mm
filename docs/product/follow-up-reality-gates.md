...
- `implemented`: M0.5 rejects enabled dashboard configurations whose host is
  not localhost or a loopback IP. The HTTP surface still has no authentication,
  so remote/public exposure remains outside the supported product boundary.
- `partial`: the MCP interface now has scoped Event/Claim search and Event get
  slices. Claim get, episode/reflection records, complete history, and audited
...
| `M0` | M0.5 Runtime Boundary and CI | `implemented` | Enabled dashboard hosts are loopback-only while auth is unavailable; CLI tracing writes to stderr; Rust 1.95.0 is pinned; Linux/macOS CI runs format, all-feature Clippy, full tests, and status sync. The isolated `rmcp 2.2.0` probe is no-go for an in-place upgrade because one handler error contract regressed. | Preserve stdout protocol isolation and loopback validation. Keep Windows in M2 and move any `rmcp` upgrade to a dedicated capability-neutral migration slice. | dashboard config positive/negative tests; bootstrap stderr/stdout integration test; `tests/ci_contract.rs`; pinned-toolchain local full gate; `docs/spikes/rmcp-compatibility-2026-07-14.md`. |
| `M1` | M1.1.1 Scoped Event Recall Read Model | `implemented` | The additive `search_memory` path requires an explicit namespace and returns bounded recent-first complete event records with stable IDs, timestamps, scope, kind, summary, and persisted claim/episode links. This slice remains Event-only and is not a complete cross-record read model, text search, history, or correction flow. | Preserve SQL-level owner+namespace filtering before limit, exact-ID no-widening, provider-independent reads, and semantic-table read-only behavior. Claim search is tracked separately in M1.1.2; keep episode/reflection/history/supersession work in later slices. | focused `sqlite_store` event-recall tests; focused `mcp_stdio` search tests including reconnect with an offline provider; `cargo test --test status_sync`; `./scripts/status-sync-check.sh`. |
| `M1` | M1.1.2 Scoped Claim Provenance Read | `implemented` | Additive `search_memory(record_type = Claim)` requires an explicit namespace, defaults omitted claim status to Active, and supports canonical/raw claim reference, status, mode, and a bounded limit. SQL narrows by owner+namespace before filters/limit; results include canonical claim/evidence references, episode references, and direct source/superseded reflection links. Claims have no stored `recorded_at`, so Event/time filters are rejected. | Preserve the default Event behavior when `record_type` is omitted, fail closed across scopes, and keep the path read-only and provider-free. Do not claim `get_memory` Claim lookup, complete reflection history, correction, or the M1 exit gate. | `cargo test --test sqlite_store sqlite_claim_recall_is_scoped_status_aware_and_returns_provenance -v`; focused `domain_snapshot` claim-reference tests; `cargo test --test mcp_stdio search_memory_returns_scoped_claims_with_revision_provenance_over_stdio -v`; `cargo test --test status_sync -v`; `./scripts/status-sync-check.sh`. |
| `M1` | M1.2.1 Scoped Event Lookup | `implemented` | The additive `get_memory` tool resolves one stable event ID only inside the required namespace and returns the same complete event/provenance shape as search. Missing and cross-scope IDs both return `record: null`; other record types and history are not implemented. | Preserve the shared scoped read service and null/no-widening behavior; do not turn lookup misses into unscoped existence probes. | focused `mcp_stdio` get-memory, tool-list, and schema tests; `cargo test --test status_sync`; `./scripts/status-sync-check.sh`. |
