...
- Rust MCP `stdio` 服务与 8 个工具；`search_memory` 已覆盖显式 namespace 的 Event、Claim 与 scoped Episode / Reflection provenance recall 首片，`get_memory` 已覆盖 Event、Claim 与 Episode lookup 首片，`get_reflection_history` 已覆盖 exact scoped Claim revision chain 首片，`get_evidence_relation` 已覆盖 scoped evidence-relation runtime 首片；
...
- 已有正式的 Event / Claim `search_memory` 与 `get_memory` 首片、Episode / Reflection `search_memory` provenance 首片、Episode `get_memory`、跨类型 `record_types` union、Claim-linked `get_reflection_history` 首片，以及 scoped `get_evidence_relation` 首片；identity/commitment history、record-only reflection、Reflection lookup 与 correction 接口仍未实现；
...
已完成十三片（截至 2026-08-13）：`M1.0.1`–`M1.0.3` 三项 Scope/Data-Integrity Gates、`M1.1.1` Event / `M1.1.2` Claim / `M1.1.3` Episode / `M1.1.4` Reflection scoped search、`M1.1.5` scoped evidence-relation runtime、`M1.1.6` 跨类型 union，以及 Event/Claim/Episode lookup 与 Claim reflection history。Event / Claim search 和 lookup 继续使用显式 namespace、SQL scope-first filtering、provider-free read 与跨 scope empty/null；Episode search 与 lookup 以同 scope Event membership 投影 Episode，并把 persisted reference 当作 opaque exact ID。Reflection search 只通过同 scope Claim 端点归属，隐藏 mixed-scope edge，并排除 record-only 行。additive `record_types` 在同一 scope 内合并已稳定的四类 tagged record，并按 recorded_at / type / id 收口。第 8 个 MCP 工具 `get_evidence_relation` 把既有只读 projection 收敛为 scoped trigger-window ∩ selected-subset 合同。`get_memory` 仍不接受 Reflection。完整 M1、identity/commitment history、record-only reflection、Reflection lookup、correction 和真实客户端退出门仍开放。

计划能力：

- M1.0 前置门、四类 scoped search、evidence-relation runtime、跨类型 union 与 Episode lookup 已通过；下一步完成 Reflection lookup 与其余 history；
- 已完成 scoped Episode / Reflection provenance search、evidence-relation runtime、跨类型 union 与 Episode lookup 首片；下一步完成 Reflection lookup；
- `search_memory`；
- `get_memory`：当前 Event / Claim / Episode；Reflection lookup 仍单独开放；
...
冻结顺序：`Reflection lookup 与其余 history → audited supersede → current-schema structural readback → real-client closure`。
