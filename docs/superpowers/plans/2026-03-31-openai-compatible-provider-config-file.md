...
- [x] **Step 1: 写失败测试，固定配置文件加载契约**
- [x] **Step 2: 运行配置相关测试，确认红测成立**
- [x] **Step 3: 实现 TOML 配置加载、默认路径与 `AGENT_LLM_MM_CONFIG` 覆盖**
- [x] **Step 4: 运行配置相关测试，确认绿测通过**
...
- [x] **Step 1: 写失败测试，固定 provider 请求/响应与错误映射契约**
- [x] **Step 2: 运行 provider 单测，确认红测成立**
- [x] **Step 3: 实现 `openai_compatible` 适配器，保留 `mock` 作为显式 provider**
- [x] **Step 4: 运行 provider 单测，确认绿测通过**
...
- [x] **Step 1: 写失败测试，固定 runtime/doctor 的 provider 选择行为**
- [x] **Step 2: 运行相关测试，确认红测成立**
- [x] **Step 3: 实现 provider 工厂与 doctor 安全输出**
- [x] **Step 4: 运行相关测试，确认绿测通过**
...
- [x] **Step 1: 写文档和样例期望，明确配置文件路径与 provider 选择方式**
- [x] **Step 2: 实现脚本透传与文档更新**
- [x] **Step 3: 自检文档命令、路径与示例一致性**
...
- [x] **Step 1: 确保 `agent-llm-mm.local.toml` 被忽略且示例配置不含真实密钥**
- [x] **Step 2: 扫描 docs/examples/tests/scripts，确认无真实 API key、token、私有 endpoint**
- [x] **Step 3: 运行最终验证：`cargo test`、`pwsh -File .\\scripts\\agent-llm-mm.ps1 doctor`（mock 与配置文件路径）**
- [x] **Step 4: 总结剩余风险，只保留真实未完成项**
