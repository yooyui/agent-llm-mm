...
| --- | --- | --- | --- | --- | --- | --- |
| 最小主链路 | 稳定打通 `events -> claims -> self_snapshot -> decision -> reflection` | 已打通完整最小闭环，并作为当前仓库主能力存在 | `已完成` | `project-status.md` 明确列出主链路；`cargo test` 全量通过 `170` 个测试 | 主要缺口不在主链路存在性，而在 richer 语义与自动化范围 | 维持现有测试基线，避免后续扩展时破坏最小闭环 |
...
| production dashboard service | 通过配置开关提供随 `serve` 启动的本机只读运行观测面板 | 已实现 `[dashboard]` 配置、bounded in-memory operation recorder、durable `operation_log` history JSON API、HTTP JSON API、SSE、静态生成图物料和 `Memory-chan Live Desk` 二次元风格页面；dashboard 详情已暴露 MCP tool call correlation id | `已完成` | `README.md`、`project-status.md`、`development-macos.md`、`testing-guide-2026-03-24.md`、`NOTICE`；`tests/dashboard_*`、`tests/mcp_stdio.rs` 的 dashboard/correlation/durable-history 测试和 MCP stdout safety 测试 | 不是远程管理后台或写入界面；暂无认证、多租户；失败路径 durable operation-log 记录仍需后续收口 | 维持只读边界；下一步做 observe-only daemon 诊断和失败路径 durable operation-log 收口 |
