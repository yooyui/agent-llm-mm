...
| --- | --- | --- | --- | --- | --- | --- |
| 最小主链路 | 稳定打通 `events -> claims -> self_snapshot -> decision -> reflection` | 已打通完整最小闭环，并作为当前仓库主能力存在 | `已完成` | `project-status.md` 明确列出主链路；`cargo test` 全量通过 `190` 个测试 | 主要缺口不在主链路存在性，而在 richer 语义与自动化范围 | 维持现有测试基线，避免后续扩展时破坏最小闭环 |
...
| 持续后台自治与完整 self-governing agent | 形成持续运行、统一触发、非手工驱动的更完整自治行为 | 当前没有后台 daemon、定时自治进程或“所有入口统一自动反思”的运行形态；observe-only daemon gate 已定义为未来 daemon 工作的前置边界 | `未实现` | `project-status.md`、`roadmap.md` 都明确否定当前已实现该能力；`docs/product/daemon-observe-only-gate.md` 定义 Local Alpha 阶段不调用 `run_reflection`、不写 identity / commitments、不启动 remote listener | 缺 daemon 化调度、统一自动触发入口、持续运行治理策略；写能力必须等单独 gate | 明确保留为后期方向；下一步只允许在 observe-only gate 内做诊断实现 |
| 产品化封装 | 从已验证 MVP 推进到 local-first formal product，并为后续 beta / remote / GA 建立 gate | MVP release gate 已过；正式产品化路线图已落到 `docs/superpowers/plans/2026-05-09-productization-roadmap.md`；Local Alpha 的 PRD、release gate、config profiles、product smoke、data safety、首版 local support bundle generator、daemon observe-only gate、correlation id contract、durable operation-log dashboard history 和失败路径 operation-log metadata 已形成首轮产品边界 | `部分完成` | `README.md`、`roadmap.md`、`release-readiness.md`、`docs/project-status.md`、`docs/product/prd-local-alpha.md`、`docs/product/release-gate-local-alpha.md`、`docs/product/support-bundle-local-alpha.md`、`docs/product/daemon-observe-only-gate.md`、`docs/product/correlation-id-contract.md`、`docs/superpowers/plans/2026-05-09-productization-roadmap.md`、`tests/support_bundle.rs`、`scripts/generate-support-bundle.sh`、`tests/mcp_stdio.rs` | 仍缺 observe-only daemon diagnostics、fresh full Local Alpha gate 证据、auth/remote/team mode/GA gate；support bundle 仍不含 log excerpts | 下一步评估 observe-only daemon 诊断实现 |
