...
| `doctor` redaction behavior | `doctor` may expose provider, base URL shape, model, provider matrix, status, and runtime readiness, but must not expose API keys, URL userinfo, query values, or equivalent secrets. | existing | `tests/provider_config.rs::doctor_report_does_not_contain_api_key_in_serialized_output` and `tests/provider_config.rs::doctor_reports_openrouter_provider_without_exposing_api_key` assert provider secrets are absent from serialized reports. |
...
- serialized decision responses and diagnostics must not expose provider API
  keys, URL userinfo, query secrets, or raw provider error payloads
