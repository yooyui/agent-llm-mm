...
- trigger-ledger-backed automatic self-revision MVP
  - 当前唯一 MCP-wired automatic path 是 `ingest_interaction -> failure trigger`
  - ingest DTO 支持传入 `trigger_hints`
...
- 但当前真正接到 MCP entry point 的只有 `ingest_interaction -> failure trigger`
...
- 无更多 provider 类型
- automatic self-revision 当前唯一 MCP-wired automatic path 是 `ingest_interaction -> failure trigger`
- `conflict` 与 `periodic` 目前只存在于 domain / coordinator / ledger 契约里，不应描述成已接到 MCP entry point
