# 本机 MCP 接入说明（2026-03-26，按 2026-04-19 fresh 验证更新）
...
截至 `2026-04-19`，fresh 验证还包括：

- `cargo test -q` 全量通过，97 个测试通过
- 其中 `application_use_cases` 20、`failure_modes` 11、`mcp_stdio` 16、`sqlite_store` 15
...
- `decide_with_snapshot` 还要求显式传 conflict-style `trigger_hints`，否则不会因为“库里已有 evidence”而自动进入 conflict self-revision
  - proposal 首阶段已可携带 `proposed_evidence_event_ids`、`proposed_evidence_query`、`confidence`，但这仍只是治理前 contract，不是 richer widening / ranking engine
...
- 当前 MCP runtime coverage 已接到这 3 条 hook，但仍是受限、显式、best-effort 的 demo 形态
...
- 无更多 provider 类型
- automatic self-revision 当前 MCP-wired automatic path 仅限 `ingest_interaction -> failure`、`decide_with_snapshot -> conflict`、`build_self_snapshot -> periodic`
- `decide_with_snapshot` / `build_self_snapshot` 仍要求显式 `auto_reflect_namespace`；`decide_with_snapshot` 还要求显式 conflict-style `trigger_hints`，并且只在非 blocked 决策后 best-effort 运行
